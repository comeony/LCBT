...
def eval_policy(policy_, env_name, seed, eval_episodes=10):
    ...
    for _ in range(eval_episodes):
        ...
        while not done:
            action = policy_.select_action(np.array(state))
...
def train(args, attacker=None):
    ...
    for t in range(int(args.max_timesteps)):
        ...
        if t < args.start_timesteps:
            ...
            if t % 10000 == 0:
                args.expl_noise = max(args.expl_noise - 0.04, 0.01)
...
def test(args):
    ...
    policy.load("./TargetModel/CarFindFlag3MEnv/DDPG/target4")
    for i in range(10):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_timesteps", default=3e8, type=int)  # Max time steps to run environment
    ...
    parser.add_argument("--attack_random_seed", default=2, type=int)  # Sets Gym, PyTorch and Numpy seeds
    parser.add_argument('--multiples_of_v', default=1, type=int)
    parser.add_argument('--alpha', default=5, type=float)
    parser.add_argument('--beginAttackK', default=500, type=int)
    ...
    if args.ATTACK:
        ...
        attack_target_model = args.attack_target_model + args.env_name + "/" + args.policy + "/target5"
