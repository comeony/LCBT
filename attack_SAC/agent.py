...
from utils import grad_false, hard_update, soft_update, to_batch,\
    update_params, RunningMeanStats,get_output_folder
...
class SacAgent:
        ...
        while True:
            ...
            if self.steps % 200000 == 0:
                ...
                if self.attacker != None:
                    np.save(self.log_dir + "/sim.npy", np.array(self.attacker.similarity))
                np.save(self.log_dir + "/reward.npy", np.array(self.t_r))
    ...
    def train_episode(self):
        ...
        while not done:
            ...
            if self.attacker != None:
                tarAction, wh = self.attacker.antiAction(action, episode_steps, state)

            next_state, reward, done, _ = self.env.step(tarAction)
            self.steps += 1
            episode_steps += 1
        ...
        self.t_r.append(episode_reward)

        if self.episodes % self.log_interval == 0:
            self.writer.add_scalar(
                'reward/train', self.train_rewards.get(), self.steps)
        if self.episodes % 2000 == 0:
            print(f'episode: {self.episodes:<4}  '
                  f'episode steps: {episode_steps:<4}  '
                  f'reward: {episode_reward:<5.1f}')

        if self.attacker != None:
            self.attacker.update(tarj)
            tarj = []
    ...
    def getDistance(self, a, b):
        ...
        return dist
    def evaluate(self, attack = None, ra = 100):
        episodes = 1000
        ...
        for i in range(episodes):
            ...
            while not done:
                ...
                if attack != None:
                    attack_action = attack.exploit(state)
                    total_step += 1
                    if self.getDistance(action, attack_action) < ra:
                        sim_step += 1
    ...
    def load_model(self,path):
        ...
        self.critic_target.load_state_dict(torch.load(path+'critic_target.pth'))
